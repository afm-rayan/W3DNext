...
void *MilesAudioManager::getHandleForBink()
{
	if (m_binkHandle == nullptr) {
		...
		aud->m_audioEventRTS = RefCountPtr<DynamicAudioEventRTS>::Create_NoAddRef(newInstance(DynamicAudioEventRTS)("BinkHandle"));
		getInfoForAudioEvent(aud->m_audioEventRTS.Peek());
		aud->m_sample = getFirst2DSample(aud->m_audioEventRTS.Peek());
